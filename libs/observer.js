...
EventBroker.prototype.subscribe = function(subject,callback, context){
  this.get(subject).push({ fn: callback, ctx: context });
};
...
EventBroker.prototype.publish = function(subject){
  ...
  for (var i = -1, len = subscribers.length; ++i < len;) {
	  subscribers[i].fn.apply(subscribers[i].ctx, args);
    //Function.prototype.bind.apply(subscribers[i], args);   
  };
};
